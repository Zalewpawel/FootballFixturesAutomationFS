...
def main():
    ...
    try:
        ...
        logging.critical(
            f"Nie można wczytać pliku {INPUT_PATH}: {e}. Zatrzymuję skrypt."
        )
        return
    try:
        ...
        logging.critical(
            f"Nie można wczytać pliku {OUTPUT_PATH}: {e}. Zatrzymuję skrypt."
        )
        return
    coord_lookup = {
        item["leagueName"]: {
    ...
    logging.info("=" * 60)
    logging.info(
        f"Pobrano {len(results_data)} lig z Robota. Rozpoczynam przetwarzanie..."
    )
    logging.info("=" * 60)
    for league_result in results_data:
        ...
        if league_name not in coord_lookup:
            logging.warning(
                f"Brak współrzędnych dla '{league_name}' w {INPUT_PATH}. Pomijam tę ligę."
            )
            continue
        ...
        if not meteo_data:
            logging.warning(
                f"Nie udało się pobrać danych pogodowych dla '{league_name}'. Pomijam tę ligę."
            )
            continue
        ...
        try:
            ...
            logging.error(
                f"Nie można utworzyć folderu '{folder_path}': {e}. Pomijam tę ligę."
            )
            continue
        ...
        logging.info(
            f"Temperatura: {weather['temperature_celsius']}°C / {weather['temperature_fahrenheit']}°F"
        )
